package org.example;

public class Calculator {
//    private boolean status = false;
//
//    public boolean isOn() {
//        return this.status;
//    }
//
//    public void turnOn(){
//        this.status = true;
//    }
//
//    public void turnOff(){
//        this.status = false;
//    }
//
//    public boolean isOff(){
//        return !this.status;
//    }

    public int sum(int sum1, int sum2){
        return sum1 + sum2;
    }

    public float sum(float sum1, float sum2){
        return sum1 + sum2;
    }

    public double sum(double sum1, double sum2){
        return sum1 + sum2;
    }

    public long sum(long sum1, long sum2){
        return sum1 + sum2;
    }

    public short sum(short sum1, short sum2){
        return (short) (sum1 + sum2);
    }

    public int difference(int sum1, int sum2){
        return sum1 - sum2;
    }

    public float difference(float sum1, float sum2){
        return sum1 - sum2;
    }

    public double difference(double sum1, double sum2){
        return sum1 - sum2;
    }

    public long difference(long sum1, long sum2){
        return sum1 - sum2;
    }

    public short difference(short sum1, short sum2){
        return (short) (sum1 - sum2);
    }

    public int multiply(int sum1, int sum2){
        return sum1 * sum2;
    }

    public float multiply(float sum1, float sum2){
        return sum1 * sum2;
    }

    public double multiply(double sum1, double sum2){
        return sum1 * sum2;
    }

    public long multiply(long sum1, long sum2){
        return sum1 * sum2;
    }

    public short multiply(short sum1, short sum2){
        return (short) (sum1 * sum2);
    }
}
